...
import { REGIONS, URUSAN, STATUS_META } from "../lib/constants";
...
import {
  Plus, FileUp, CheckCircle2, AlertTriangle, Eye, Loader2, Send, FileText, Building2,
} from "lucide-react";
...
export default function PerangkatDashboard() {
  ...
  const [form, setForm] = useState({ area: "", device_name: "", urusan: "" });
  ...
  const load = useCallback(async () => {
    ...
    setStats(st.data);
    if (detail) {
      const found = s.data.find((x) => x.id === detail.id);
  ...
  const createSubmission = async () => {
    if (!form.area || !form.device_name || !form.urusan) return toast.error("Lengkapi semua isian");
    if (!period) return toast.error("Belum ada periode evaluasi aktif");
    try {
      const { data } = await api.post("/submissions", { ...form, period_id: period.id });
      toast.success("Pengajuan dibuat, silakan unggah berkas");
      setCreateOpen(false);
      setForm({ area: "", device_name: "", urusan: "" });
      await load();
      setDetail(data);
    } catch (e) {
  ...
  const submitFinal = async () => {
    try {
      await api.post(`/submissions/${detail.id}/submit`);
      toast.success("Pengajuan dikirim untuk verifikasi");
      await load();
    } catch (e) {
      toast.error(formatApiErrorDetail(e.response?.data?.detail));
    }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Ajukan dan pantau evaluasi kematangan kelembagaan Anda.</p>
          </div>
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger asChild>
              <Button data-testid="new-submission-btn" className="gap-2 h-11" disabled={period?.upload_locked}>
                <Plus className="w-4 h-4" /> Ajukan Evaluasi Baru
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Pengajuan Evaluasi Baru</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 py-2">
                <div className="space-y-2">
                  <Label>Area (Kabupaten/Kota)</Label>
                  <Select value={form.area} onValueChange={(v) => setForm({ ...form, area: v })}>
                    <SelectTrigger data-testid="submission-area-select"><SelectValue placeholder="Pilih area" /></SelectTrigger>
                    <SelectContent>{REGIONS.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label>Nama Perangkat Daerah</Label>
                  <Input data-testid="submission-device-input" placeholder="mis. Dinas Kominfo" value={form.device_name} onChange={(e) => setForm({ ...form, device_name: e.target.value })} />
                </div>
                <div className="space-y-2">
                  <Label>Urusan</Label>
                  <Select value={form.urusan} onValueChange={(v) => setForm({ ...form, urusan: v })}>
              ...
              <DialogFooter>
                <Button data-testid="submission-create-confirm" onClick={createSubmission}>Buat Pengajuan</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {subs.length === 0 && (
            <div className="col-span-full text-center py-16 rounded-2xl border border-dashed border-border">
              ...
              <p className="text-muted-foreground mt-3">Belum ada pengajuan. Klik "Ajukan Evaluasi Baru".</p>
            </div>
          )}
          {subs.map((s) => (
            <div key={s.id} data-testid={`submission-card-${s.id}`} className="rounded-2xl border border-border bg-white p-5 hover:shadow-md transition-shadow">
              ...
              {s.status === "ditolak" && s.rejection_note && (
                <div className="rounded-lg bg-red-50 border border-red-200 px-3 py-2 text-xs text-red-700 mb-3">
                  <span className="font-semibold">Catatan perbaikan: </span>{s.rejection_note}
                </div>
              )}
              <Button variant="outline" size="sm" className="w-full gap-2" data-testid={`open-submission-${s.id}`} onClick={() => setDetail(s)}>
                <Eye className="w-4 h-4" /> {s.status === "selesai" ? "Lihat Hasil" : editable ? "Kelola" : "Lihat Detail"}
              </Button>
            </div>
          ))}
        </div>
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          {detail && (
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">{detail.device_name}</DialogTitle>
              </DialogHeader>
              <div className="text-xs text-muted-foreground -mt-2">{detail.area} · {detail.urusan} · Tahun {detail.year}</div>
              ...
              {detail.status === "ditolak" && detail.rejection_note && (
                <div className="rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700 mb-2">
                  ...
                  <p className="mt-1 text-xs">Perbaiki berkas terkait lalu kirim ulang.</p>
                </div>
              )}

              {detail.status === "selesai" && detail.scoring ? (
                <MaturityReport submission={detail} />
              ) : (
                <div className="space-y-2">
                  <div className="text-sm font-semibold text-slate-900">Berkas Pendukung per Variabel (Permendagri 99/2018)</div>
                  {indicators.map((ind) => {
                      ...
                      <div key={ind.id} className="rounded-lg border border-border p-3 flex items-center justify-between gap-3">
                        ...
                        {editable && (
                          <label className="shrink-0">
                            <input type="file" className="hidden" data-testid={`upload-input-${ind.id}`}
                              onChange={(e) => uploadFor(ind.id, e.target.files[0])} />
                            <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border cursor-pointer transition-colors ${up ? "border-slate-300 text-slate-600 hover:bg-slate-50" : "border-primary text-primary hover:bg-primary hover:text-white"}`}>
                              {uploading === ind.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FileUp className="w-3.5 h-3.5" />}
                              {up ? "Ganti" : "Unggah"}
                            </span>
                          </label>
                        )}
                        {!editable && up && (
                          <Button variant="ghost" size="sm" onClick={() => openFile(up.file_id)}><Eye className="w-4 h-4" /></Button>
                        )}
                      </div>
                    );
                  })}
                  {editable && (
                    <Button className="w-full gap-2 mt-3 h-11" data-testid="submit-submission-btn" onClick={submitFinal}>
                      <Send className="w-4 h-4" /> Kirim untuk Verifikasi
                    </Button>
                  )}
                  {(detail.status === "menunggu_verifikasi" || detail.status === "menunggu_penilaian") && (
                    <div className="rounded-lg bg-blue-50 border border-blue-200 px-4 py-3 text-sm text-blue-700 mt-2">
                      Pengajuan sedang {STATUS_META[detail.status].label.toLowerCase()}. Mohon menunggu.
                    </div>
                  )}
                </div>
              )}
